param(
    [string[]]$Target,
    [switch]$Debug,
    [string]$Image = "pemu-ps4-builder:latest",
    [switch]$NoBuildImage,
    [switch]$Help
)
...
function Show-Usage {
    @"
Usage: powershell -ExecutionPolicy Bypass -File .\scripts\build-ps4.ps1 [options]

Build pEMU PS4 packages in Docker on a Windows host.
...
Examples:
  ...
  powershell -ExecutionPolicy Bypass -File .\scripts\build-ps4.ps1 -Target pfbneo -Target pgba -Debug
...
try {
    ...
    $containerArgs = @("./scripts/build-docker-target.sh", "--platform", "ps4")
    if ($Debug) {
        $containerArgs += "--debug"
    }
    if ($Target) {
